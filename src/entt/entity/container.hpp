#ifndef ENTT_ENTITY_CONTAINER_HPP
#define ENTT_ENTITY_CONTAINER_HPP
...
namespace entt {
	...
	*
	* The container is the core class of the entity-component framework.<br/>
	* It stores entities and arranges pools of components on a per request basis.
	* By means of a container, users can manage entities and components and thus
	* create views to iterate them.
	*
	* @tparam Entity A valid entity type (see entt_traits for more details).
	*/
	template<typename Entity, typename TagFamily, typename ComponentFamily, typename ViewFamily>
	class Container {
		using tag_family = Family<TagFamily>;
		using component_family = Family<ComponentFamily>;
		using view_family = Family<ViewFamily>;
		...
		struct Pool : SparseSet<Entity, Component> {
			using test_fn_type = bool(Container::*)(Entity) const;

			template<typename... Args>
			Component & construct(Container &container, Entity entity, Args &&... args) {
				auto &component = SparseSet<Entity, Component>::construct(entity, std::forward<Args>(args)...);

				for (auto &&listener : listeners) {
					if ((container.*listener.second)(entity)) {
		...
		Pool<Component> & pool() noexcept {
			return const_cast<Pool<Component> &>(const_cast<const Container *>(this)->pool<Component>());
		}
		...
		SparseSet<Entity> & handler() {
			...
			if (!handlers[vtype]) {
				...
				accumulator_type accumulator = {
					(ensure<Component>().append(set.get(), &Container::has<Component...>), 0)...
				};
	...
	public:
		...
		/*! @brief Copying a container isn't allowed. */
		Container(const Container &) = delete;
		/*! @brief Default move constructor. */
		Container(Container &&) = default;

		/*! @brief Copying a container isn't allowed. @return This container. */
		Container & operator=(const Container &) = delete;
		/*! @brief Default move assignment operator. @return This container. */
		...
		* container could decide to prepare internal data structures for it for
		* later uses.<br/>
		...
		*/
		...
		/**
		* @brief Increases the capacity of a container in terms of entities.
		*
		* If the new capacity is greater than the current capacity, new storage is
		* allocated, otherwise the method does nothing.
		*
		...
		*
		* @warning
		* Attempting to use an entity that doesn't belong to the container results
		* in undefined behavior. An entity belongs to the container even if it has
		* been previously destroyed and/or recycled.<br/>
		* An assertion will abort the execution at runtime in debug mode if the
		* container doesn't own the given entity.
		*
		...
		*/
		...
		/**
		* @brief Destroys an entity and lets the container recycle the identifier.
		*
		...
		*/
		template<typename Tag>
		Tag & get() noexcept {
			return const_cast<Tag &>(const_cast<const Container *>(this)->get<Tag>());
		}
		...
		*/
		...
		Component & get(entity_type entity) noexcept {
			return const_cast<Component &>(const_cast<const Container *>(this)->get<Component>(entity));
		}
		...
		*
		* @code{.cpp}
		* if(container.has<Component>(entity)) {
		*     container.replace<Component>(entity, args...);
		* } else {
		*     container.assign<Component>(entity, args...);
		* }
		* @endcode
		*
		...
		*/
		...
		/**
		* @brief Resets a whole container.
		*
		...
		*
		* This kind of views are created on the fly and share with the container its
		...
		*
		* @see View
		* @see View<Entity, TagFamily, ComponentFamily, ViewFamily, Component>
		* @see PersistentView
		*
		...
		*/
		template<typename... Component>
		View<Entity, TagFamily, ComponentFamily, ViewFamily, Component...> view() {
			return View<Entity, TagFamily, ComponentFamily, ViewFamily, Component...>{ensure<Component>()...};
		}
		...
		* internal data structures of the container that are dedicated to this kind
		...
		* is allocated within the container and it increases memory pressure.
		...
		* data structures used by persistent views when the container is still
		...
		*/
		template<typename... Component>
		PersistentView<Entity, TagFamily, ComponentFamily, ViewFamily, Component...> persistent() {
			// after the calls to handler, pools have already been created
			return PersistentView<Entity, TagFamily, ComponentFamily, ViewFamily, Component...>{handler<Component...>(), pool<Component>()...};
		}
...
#endif // ENTT_ENTITY_CONTAINER_HPP
